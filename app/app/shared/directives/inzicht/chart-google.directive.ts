    ...
    controller: function($scope, $element, $injector, buildChart, FilterService, pivotTable, jsonToArray, NULL_REPLACER, stripData) {
      ...
      $scope.selectItem = function(item) {
        ...
        if (filterKeyType === "object") {
          ...
          if (isCrap(operator)) operator = '=';
        } else {
          propertyKey = filterKey;
          operator = '=';
        }

        var FilterKeyIndex: number;

        // Als er niet gedeselecteerd is dan is er dus geselecteerd
        if (item.selectedItem !== undefined) {
          if (propertyKey !== undefined) {

            // Get index van te gebruiken FilterKey
            for(var i = 0; i < item.data[0].length; i++ ) {
              if(item.data[0][i].toLowerCase() === propertyKey.toLowerCase() ) {
                FilterKeyIndex = i
              }
            }
            if (FilterKeyIndex === undefined || FilterKeyIndex === null) { FilterKeyIndex = 0 }

            var selectedItem = item.selectedItem,
              row = selectedItem.row + 1,
              column = selectedItem.column,
              propertyValue = item.data[row][FilterKeyIndex], // Gebruikt nu opgezochte FilterKeyIndex van headers
              restValue = item.data[row][0];
      ...
      $scope.updateChart = function(data) {
        ...
        if (chartType === 'SuperPie') chartType = 'PieChart';

        // Naar array van arrays voor google
        ...
        if (amountResults === 0 || amountResults === undefined || amountResults === null) amountResults = 10;
        ...
        if(restDataTotal[1] > 0) primaryChartData.push(restDataTotal);
        if (chartType === "Table") primaryChartData.push(total);

        $scope.chartData = buildChart(primaryChartData, chartType, $scope.columns, $scope.options);

        $scope.setLoading(false); // hide loading animation
